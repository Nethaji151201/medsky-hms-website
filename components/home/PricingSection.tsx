...
function PricingCard({ tier, categoryKey }: { tier: PricingTier; categoryKey: PricingCategoryKey }) {
  ...
  return (
    <div
      className={`w-full flex flex-col justify-between bg-white dark:bg-[#0f172a] rounded-[22px] p-6 sm:p-7 border transition-all duration-300 select-none relative ${tier.isPopular
    ...
    >
      ...
      {(tier.badge || tier.isPopular) && (
        <div className="absolute -top-3.5 right-6 z-20">
          <span className="px-3.5 py-1 rounded-full bg-primary text-white text-[11px] font-extrabold uppercase tracking-wider shadow-md shadow-primary/30 inline-block">
            {tier.badge || "POPULAR"}
          </span>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-full overflow-hidden bg-cyan-50 dark:bg-slate-800 border border-cyan-100 dark:border-slate-700 flex-shrink-0 flex items-center justify-center text-primary">
        ...
        <div className="pt-3 pb-2 border-y border-slate-100 dark:border-slate-800/80">
          ...
          <div className="text-[11.5px] text-slate-400 dark:text-slate-400 mt-1.5 flex items-center justify-between">
            <span className="font-semibold text-accent-foreground dark:text-cyan-400">✓ {tier.billingPeriodNote}</span>
      ...
      <div className="pt-6 mt-3">
        <Link
          href={tier.ctaHref}
          className={`w-full py-3.5 px-6 rounded-full font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all duration-200 shadow-sm ${isTealButton
            ? "bg-primary hover:bg-primary-hover text-white shadow-md shadow-primary/25 active:scale-[0.98]"
...
export function PricingSection({ showHeader = true }: { showHeader?: boolean }) {
  ...
  return (
    <section
      className="py-20 sm:py-28 relative overflow-hidden bg-white dark:bg-[#060b14] gpu-layer"
    >
